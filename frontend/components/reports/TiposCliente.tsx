...
export function TiposCliente({ salesHook, dateRange }: TiposClienteProps) {
  ...
  // Procesar datos de tipos de cliente
  const datosTiposCliente = useMemo(() => {
    if (!dateRange) return [];
    ...
    ventasFiltradas.forEach(venta => {
      const tipoCliente = venta.tipoCliente || 'Sin tipo';

      if (!agrupados[tipoCliente]) {
        agrupados[tipoCliente] = {
          monto: 0,
          transacciones: 0
        };
      }

      agrupados[tipoCliente].monto += venta.totalOrden;
      agrupados[tipoCliente].transacciones += 1;
    });
    ...
    const resultado: TipoClienteData[] = Object.entries(agrupados).map(([tipoCliente, datos]) => ({
      tipoCliente,
      monto: datos.monto,
      porcentaje: (datos.monto / montoTotal) * 100,
      transacciones: datos.transacciones,
      color: CLIENT_COLORS[tipoCliente] || '#757575'
    }));
  ...
  const montoMaximo = Math.max(...datosTiposCliente.map(item => item.monto), 1);

  return (
    <Card>
      <CardHeader>
        <div>
          <CardTitle className="text-lg">Ventas por Tipo de Cliente</CardTitle>
      ...
      <CardContent>
        {datosTiposCliente.length === 0 ? (
          <div className="h-64 flex flex-col items-center justify-center text-gray-500">
            <div className="text-center">
              <div className="text-4xl mb-4">👥</div>
              <h3 className="text-lg font-medium mb-2">No hay datos de tipos de cliente</h3>
              <p className="text-sm">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              {datosTiposCliente.map((item, index) => (
                <div key={item.tipoCliente} className="space-y-1">
                  {/* Nombre del tipo de cliente */}
                  <div className="flex justify-between items-center text-sm">
                    <span className="font-medium text-gray-700">{item.tipoCliente}</span>
